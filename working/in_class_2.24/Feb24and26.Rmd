...
resl=as.data.frame(res) %>%
  gather(cdrag, power) %>% 
  mutate(mean_speed = rep(100, length(res)))
results_100 <- ggplot(resl, aes(cdrag, power, fill=cdrag))+geom_boxplot() + labs(y="Rangoe of Power (W/s", "Drag Coefficient")
...
```
# In-Class Activity
Reduce the mean speed to (80 km/hr) and compare to the 100 km/hr results  

```{r inclass}
#Reduce mean speed to 80
mean_speed_low = log(80*0.277)
speeds_low = rlnorm(mean=mean_speed_low, sd=0.1*mean_speed, nsample)

results_low = data.frame(speed=speeds_low, power=NA)

possible_cars$row = seq(from=1, to=nrow(possible_cars))
whichcar = base::sample(possible_cars$row, size=nsample, prob=possible_cars$prob, replace=TRUE)

# add a car to the our data structure that contains speeds
results_low$mass = possible_cars$mass[whichcar]

#  add the area
results_low$area = possible_cars$area[whichcar]

# now lets get power for all of our samples of speed and car type
results_low$power = autopower(A=results$area, V=results$speed, m=results$mass)

# create a sequence of drag efficienies
cdrag = seq(from=0.3, to=0.5, by=0.05)

#Apply over the cdrag results:
res_low = sapply(cdrag,  autopower, A=results_low$area, V=results_low$speed, m=results_low$mass)

# rearrange to plot - common way to get data into a form that works with ggplot
colnames(res_low)=cdrag
resl_low=as.data.frame(res_low) %>% 
  gather(cdrag, power) %>%
  mutate(mean_speed = rep(80, length(res_low)))

results_80 <- ggplot(resl_low, aes(cdrag, power, fill=cdrag))+geom_boxplot() + labs(y="Rangoe of Power (W/s", "Drag Coefficient")
...
##Combine results to compare on one graph:
combined_result <- rbind(resl, resl_low)

#Graph one:
results_combine <- ggplot(combined_result, aes(cdrag, power, fill=cdrag)) + geom_boxplot() + facet_wrap(~mean_speed)
results_combine
```
...
meanspeed = sum_speed/length(speeds)
mean(speeds)
```
...
npvs_edit = gather(npvs, 1:7, key=dis, value=npv)
head(npvs)
ggplot(npvs_edit, aes(x=npv, col=as.factor(dis)))+geom_density(size=2)+scale_color_brewer(type="seq", name="Discount")
 
 # how about summing all the damages
 npv.total =npvs_edit %>% group_by(dis) %>% summarize(t=sum(npv))
 ggplot(npv.total, aes(dis,t, fill=dis))+geom_col() + labs(x="Discount Rate", y="Total ($)")

  
# try use the NPV loop with a loop of times as well:
yr = c(10,15,20) #set three years
discount_rates = rep(seq(from=0.01, to=0.04, by=0.005),3) #repeat discount rates three times
npvs = as.data.frame(matrix(nrow=length(damages), ncol=(length(discount_rates))))

for(i in 1:length(damages)){
  for(j in 1:length(discount_rates)){
    for(y in 1:length(yr)){
      npvs[i,j] = compute_NPV(value= damages[i], discount=discount_rates[j], time=yr[y])
    }
  }
} 

#try this loop with a list:
...
# cross all the lists to get every combination of damage, discount, and yr
npv_input = npv %>% cross()

results = list()

for(i in 1:84){
  results[[i]] = compute_NPV(value=npv_input[[i]]$damages, discount=npv_input[[i]]$discount_rates,
                             time=npv_input[[i]]$yr)
}
results_npv <- as.data.frame(unlist(results))
```
...
```{r}
plottitle = spritf("We caught %f fish", max(summary(catch1)))
...
species_min <- which.min(summary(species))
rare <- names(summary(species)[species_min])

# output from function
return(list(num=nspecies, simpson=diversity, dominant=dominant, rare=rare))
}

#Use flower data to test the function:
library(here)
flowers <- read.table(file = file.path(here(), "working/in_class_data/flowers.txt"))

#Test function
flower_diversity <- computediversity(species = flowers$V1)
```
