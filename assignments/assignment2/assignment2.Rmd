...
test1 <- subset(catch, location == "Alaska")
test1_max <- test1$fish[which.max(test1$catch)]

for(i in 1:7){
  fish <- test1$fish[i]
  fish.price <- fish_prices$price[fish_prices$fish == fish]
  test1$price[i] <- fish.price
}
