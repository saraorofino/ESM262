...
fish_revenue <- function(fish_data, price){
  ...
  for(i in 1:length(places)){
    ...
    results$location[i] <- places[i] #set the location
    
    #Subset by location to find which fish had the highest catch for each location
    results$common_fish[i] <- fish_data$fish[which.max(fish_data$places[i])]
...
#Test:
test <- fish_revenue(fish_data = catch, price = fish_prices)
